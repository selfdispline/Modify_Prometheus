...
{
    if (msg->header.frame_id == "TFmini")
    {
        tfmini_hight = msg->range;
    }
    else
    {
        pub_message(message_pub, prometheus_msgs::Message::NORMAL, NODE_NAME, "wrong TFmini frame id.");
    }
}
...
int main(int argc, char **argv)
{
    ...
    // 【订阅】TFmini估计的高度
    ros::Subscriber tfmini_sub = nh.subscribe<sensor_msgs::Range>("/prometheus/tfmini_range", 100, tfmini_cb);
...
void pub_to_nodes(prometheus_msgs::DroneState State_from_fcu)
{
    ...
    if(input_source == 9 )
    {
        Drone_State.position[2]  = tfmini_hight;
    }
